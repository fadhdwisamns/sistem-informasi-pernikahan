...
<html lang="id">
<head>
    <meta charset="UTF-8"><title>Rekapitulasi Perceraian</title>
    <style>
        @page { margin: 0; }
...
<body>
    <div class="page">
        ...
        <div class="content">
            <h4 style="font-weight: bold; text-align: center; font-size: 14pt; margin-top: 1rem; text-decoration: underline;">REKAPITULASI DATA PERCERAIAN</h4>
            ...
            <table class="data-table" style="width: 100%; margin-top: 1rem;">
                ...
                <tbody>
                    {{-- Sesuaikan variabel $rekapPerceraian dan nama kolomnya --}}
                    @forelse($rekapPerceraian as $key => $data)
                    <tr>
                        <td style="text-align: center;">{{ $key + 1 }}</td>
                        <td>{{ $data->nomor_akta_cerai }}</td>
                        <td style="text-align: center;">{{ \Carbon\Carbon::parse($data->tanggal_putusan_cerai)->format('d-m-Y') }}</td>
                        <td>{{ $data->nama_suami }}</td>
                        <td>{{ $data->nama_istri }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" style="text-align: center; height: 50px;">Tidak ada data perceraian untuk periode ini.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
