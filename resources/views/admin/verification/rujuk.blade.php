        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <h3 class="font-semibold text-lg text-gray-900 dark:text-gray-100 mb-6">Detail Rujuk untuk Verifikasi</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <div><p class="font-medium text-gray-500 dark:text-gray-400">No Surat Rujuk:</p><p class="text-gray-900 dark:text-gray-100 mt-1">{{ $rujuk->no_surat_rujuk }}</p></div>
                        <div><p class="font-medium text-gray-500 dark:text-gray-400">Tanggal Rujuk:</p><p class="text-gray-900 dark:text-gray-100 mt-1">{{ \Carbon\Carbon::parse($rujuk->tanggal_rujuk)->translatedFormat('d F Y') }}</p></div>
                        <div><p class="font-medium text-gray-500 dark:text-gray-400">Suami:</p><p class="text-gray-900 dark:text-gray-100 mt-1">{{ $rujuk->nama_suami }} (NIK: {{ $rujuk->nik_suami }})</p></div>
                        <div><p class="font-medium text-gray-500 dark:text-gray-400">Istri:</p><p class="text-gray-900 dark:text-gray-100 mt-1">{{ $rujuk->nama_istri }} (NIK: {{ $rujuk->nik_istri }})</p></div>
                        <div class="md:col-span-2"><p class="font-medium text-gray-500 dark:text-gray-400">Tempat Rujuk:</p><p class="text-gray-900 dark:text-gray-100 mt-1">{{ $rujuk->tempat_rujuk }}</p></div>
                    </div>

                    <form method="POST" action="{{ route('admin.rujuk.verify_data', $rujuk) }}" class="space-y-6">
                        @csrf
                        @method('PATCH')
