<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(auth()->user()->role == 'admin')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg"><div class="flex items-center space-x-4"><div class="flex-shrink-0 p-3 bg-indigo-100 dark:bg-indigo-900 rounded-full"><svg class="h-7 w-7 text-indigo-500 dark:text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg></div><div><p class="text-sm text-gray-500 dark:text-gray-400">Pernikahan Bulan Ini</p><h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $pernikahan_bulan_ini }}</h3></div></div></div>
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg"><div class="flex items-center space-x-4"><div class="flex-shrink-0 p-3 bg-rose-100 dark:bg-rose-900 rounded-full"><svg class="h-7 w-7 text-rose-500 dark:text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><div><p class="text-sm text-gray-500 dark:text-gray-400">Perceraian Bulan Ini</p><h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $perceraian_bulan_ini }}</h3></div></div></div>
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg"><div class="flex items-center space-x-4"><div class="flex-shrink-0 p-3 bg-amber-100 dark:bg-amber-900 rounded-full"><svg class="h-7 w-7 text-amber-500 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg></div><div><p class="text-sm text-gray-500 dark:text-gray-400">Menunggu Verifikasi</p><h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $menunggu_verifikasi }}</h3></div></div></div>
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg"><div class="flex items-center space-x-4"><div class="flex-shrink-0 p-3 bg-cyan-100 dark:bg-cyan-900 rounded-full"><svg class="h-7 w-7 text-cyan-500 dark:text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div><div><p class="text-sm text-gray-500 dark:text-gray-400">Total KUA Terdaftar</p><h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $total_kua }}</h3></div></div></div>
                </div>
            ...
            {{-- ======================================================================= --}}
            {{-- BAGIAN BERSAMA: GRAFIK DAN AKTIVITAS (TATA LETAK BARU) --}}
            {{-- ======================================================================= --}}
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-4 mb-4">
                            Grafik Rentang Usia Pernikahan & Perceraian - {{ date('Y') }}
                        </h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Grafik Tren Bulanan
            const trenCanvasElement = document.getElementById('trenChart');
            if (trenCanvasElement) {
                const ctx = trenCanvasElement.getContext('2d');
                const trenChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        scales: {
                            y: { beginAtZero: true, grid: { color: gridColor }, ticks: { color: labelColor, callback: function(value) { if (Number.isInteger(value)) { return value; } } } },
                            x: { grid: { display: false }, ticks: { color: labelColor } }
                        },
                        plugins: { legend: { position: 'top', labels: { color: labelColor, usePointStyle: true, boxWidth: 8 } } },
            ...
            // Grafik Rentang Usia (LINE CHART)
            const ageCanvasElement = document.getElementById('ageChart');
            if (ageCanvasElement) {
                const ageCtx = ageCanvasElement.getContext('2d');
                const ageChart = new Chart(ageCtx, {
                    type: 'line',
                    data: {
                        labels: @json($age_chart_labels ?? []),
                        datasets: [
                            {
                                label: 'Pernikahan',
                                data: @json($pernikahan_age_data ?? []),
                                borderColor: 'rgba(99, 102, 241, 1)',
                                backgroundColor: 'rgba(99, 102, 241, 0.2)',
                                tension: 0.4, 
                                fill: true
                ...
                window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', event => {
                    ...
                    if(ageChart) {
                        ...
                        ageChart.options.scales.y.ticks.color = labelColor;
                        ageChart.options.scales.x.ticks.color = labelColor;
                        ageChart.options.plugins.legend.labels.color = labelColor;
                        ageChart.update();
                    }
                });
            }
